/**
 * Remove Firestore `users/{uid}` docs with no matching Auth user.
 *
 * From `functions/`:
 *   npm run prune:orphan-users -- --dry-run
 *   npm run prune:orphan-users
 *
 * Loads `.env.local` (repo root) via `./loadEnvForImport`.
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(
      "Prune failed. Use Application Default Credentials, " +
        "TRADERS_MARKET_FIREBASE_SERVICE_ACCOUNT_KEY in .env.local, " +
        "or the deployed httpPruneOrphanFirestoreUsers endpoint (URL in firebase deploy output)."
    );
    console.error(e);
    process.exit(1);
  }
